...
from .environment_variables import CALCUS_KEY_HOME, CALCUS_SCR_HOME, IS_TEST
from .helpers import (
    get_random_string,
    get_xyz_from_cube,
        ...
        if acc_type == "student":
            ...
            if form_student.is_valid():
                ...
                if user:
                    ...
                    logger.error("Could not log in student")
...
def parse_parameters(request, parameters_dict, is_flowchart=None, verify=False):
    ...
    if "calc_driver" in parameters_dict:
        ...
        if driver not in ["xtb", "gaussian", "orca", "nwchem", "pysisyphus"]:
...
def save_preset(request):
    ...
    Preset.objects.create(name=preset_name, author=request.user, params=params)
...
# UNUSED as of now, used for batch calculations
@login_required
                ...
                if len(uploaded_files) > 1:
                    if combine == "on" and parse_filenames != "on":
                        ...
                        for other_filename, other_struct in uploaded_files.items():
                            ...
                            if m != multiplicity:
                                return f"Cannot combine files with different multiplicity specifications ({filename}: multiplicity={multiplicity}, {other_filename}: multiplicity={m})"
...
def can_view_order(order, user):
    if order.author == user:
        ...
        if order.project.private and not (user.is_PI or user.professor_of):
            return False
        return True
    return False
...
def project_folders(request, user_id, proj, folder_path):
    ...
    if path[-1].strip() != "":
        return HttpResponseRedirect(f"/projects/{user_id}/{proj}/{folder_path + '/'}")
...
def webhook(request):
    ...
    if event.type == "invoice.paid":
        logger.info("Received webhook invoice.paid - Creating a subscription")
